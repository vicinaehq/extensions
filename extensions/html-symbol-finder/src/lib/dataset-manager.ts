...
import { DATASET_NAME, SEARCH_RESULT_LIMIT } from "~/config/preferences";
import type { Character, Dataset } from "~/types";

const dataset = JSON.parse(fs.readFileSync(`${environment.assetsPath}/${DATASET_NAME}.json`, "utf-8")) as Dataset;
 ...
 */
const getExactChar = (query: string): Character | null => {
 ...
 */
export const getFilteredDataset = (query: string | null, filter: string | null): Dataset => {
  ...
  if (!query) {
    return {
      ...
      characters: filter !== null ? allCharacters : allCharacters.slice(0, SEARCH_RESULT_LIMIT),
    };
  }
  ...
  const fuseResults = fuse.search(fuseSearchPattern, {
    limit: SEARCH_RESULT_LIMIT,
  });
  const characters = fuseResults.map((fuseResult) => ({
    ...fuseResult.item,
