...
import type { BlockExtra, CharAlias, Character as JSONCharacter } from "../types";
...
const allCharactersByCode = allCharacters.reduce<{
  [charCode: string]: Character;
...
const generateDataset = async (sets: CharacterSetType[], fileName: string) => {
  ...
  console.log(`i Found ${allBlocks.length} unicode blocks and ${allCharacters.length} unicode characters`);
  ...
  const dataset = {
    blocks: allBlocks,
    characters: [...extraCharacters, ...characters],
  };

  const datasetOutputPath = path.resolve(__dirname, `../../assets/${fileName}.json`);
  await writeFile(datasetOutputPath, JSON.stringify(dataset));
