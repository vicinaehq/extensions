...
describe("cacheValidation", () => {
  describe("validateCache", () => {
    ...
    describe("given cache missing eventCalendars", () => {
      const cacheData = { ...validCacheData, eventCalendars: undefined };

      describe("when validating", () => {
        it("then returns invalid with reason", () => {
          const result = validateCache(cacheData as any, validCalendars as any);
          expect(result.valid).toBe(false);
          expect(result.reason).toBe("missing eventCalendars");
        });
      });
    });

    describe("given cache missing timestamp", () => {
      const cacheData = { ...validCacheData, timestamp: undefined };
    ...
    describe("given cache with wrong version", () => {
      const cacheData = { ...validCacheData, cacheVersion: CACHE_VERSION + 1 };

      describe("when validating", () => {
        it("then returns invalid with reason", () => {
          const result = validateCache(cacheData, validCalendars as any);
          expect(result.valid).toBe(false);
    ...
    describe("given expired cache", () => {
      ...
      const cacheData = { ...validCacheData, timestamp: expiredTimestamp };
