...
export async function applyWorkflow(workflowName: string): Promise<void> {
	const result = await executeFlowwCommand(WORKFLOW_ACTIONS.APPLY, [
		workflowName,
	]);

	if (result.success) {
		await showSuccessToast(
			`${SUCCESS_MESSAGES.WORKFLOW_APPLIED}: "${workflowName}"`,
		);
	}
...
export async function validateWorkflow(
	...
	const result = await executeFlowwCommandSilent(WORKFLOW_ACTIONS.VALIDATE, [
		workflowName,
	]);
