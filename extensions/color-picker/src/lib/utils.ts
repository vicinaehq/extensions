...
import type { Colors, Palette } from "color-namer";
import uniqBy from "lodash/uniqBy";
import { formatColor } from "./color-format";
import type { ColorFormatType, CopyColorsFormat, ExtensionPreferences, HistoryColor, HistoryItem } from "./types";
...
export function getFormattedColor(_color: HistoryColor, format?: ColorFormatType) {
  ...
  return formatColor(_color, format || prefs.colorFormat || "hex");
}
...
export function getIcon(color: HistoryColor): Image.ImageLike | undefined {
  ...
  try {
    previewColor = getFormattedColor(color, "hex");
  } catch {
    previewColor = typeof color === "string" ? color : undefined;
  }
  if (!previewColor) {
    return undefined;
  }
