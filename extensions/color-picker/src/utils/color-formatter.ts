import { getPreferenceValues, Icon, Image, Keyboard, List } from "@vicinae/api";
import ColorJS from "colorjs.io";
import { HistoryColor, HistoryItem, ColorFormatType } from "@/types";

const preferences = getPreferenceValues<ExtensionPreferences>();

export function getFormattedColor(_color: HistoryColor, format?: ColorFormatType): string {
  let color;
  if (typeof _color === "string") {
    ...
    // DeprecatedColor format (0-255 range)
...
// Some formats don't work well for preview (e.g., P3 display issues)
const unsupportedPreviewFormats = ["p3", "rgb", "rgb-percentage"];

export function getPreviewColor(color: HistoryColor): string {
...
export function getShortcut(index: number): Keyboard.Shortcut | undefined {
  const key = index + 1;
...
export function getIcon(color: HistoryColor): Image.ImageLike | undefined {
...
export function getAccessories(historyItem: HistoryItem): List.Item.Accessory[] {
...
export function normalizeColorHex(colorInput: string): string {
