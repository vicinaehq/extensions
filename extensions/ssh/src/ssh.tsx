...
type Host = { host: string; subtitle: string, fromCache: boolean };
    ...
    useEffect(() => {
        getSshHostsFromConfig().then((configHosts) => {
            ...
            const uniqueHistoryHosts = historyHosts.filter(
                (h: Host) => !configHostNames.has(h.host),
            );
                            ...
                            <ActionPanel>
                                ...
                                {host.fromCache && (
                                    <Action
                                        ...
                                        icon={Icon.Trash}
                                        onAction={() => {
                                            const updated = hostsVisited.filter((h: string) => h !== host.host);
                                            cache.set("visited", JSON.stringify(updated));
                                            let updatedHosts = hosts.filter((h: Host) => h.host !== host.host);
                                            setHosts(updatedHosts);
                                            showToast({ title: `Removed ${host.host} from history` });
                                        }}
                                    />
                                )}
                            </ActionPanel>
 ...
 */
export async function getSshHostsFromConfig(): Promise<Host[]> {
        ...
        for (const line of lines) {
            ...
            if (trimmed.startsWith("Host ")) {
                ...
                for (const h of hostnames) {
                    if (h !== "*" && h.length > 0) {
                        hosts.push({ host: h, subtitle: "from .ssh/config", fromCache: false });
                    }
                }
            }
        }
