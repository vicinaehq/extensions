...
export default function WikipediaPage({ title, language }: { title: string; language: Locale }) {
  ...
  const { page, content, metadata, links } = usePageData(title, language);
  ...
  return (
    <Detail
      ...
      actions={
        <ActionPanel>
          <Action.OpenInBrowser title="Open in Browser" url={page.content_urls.desktop.page} />
          <Action
          ...
          <ActionPanel.Section>
            ...
            >
              {links?.map((link: string) => (
                <Action.Push key={link} title={link} target={<WikipediaPage title={link} language={language} />} />
              ))}
            </ActionPanel.Submenu>
          </ActionPanel.Section>
        </ActionPanel>
