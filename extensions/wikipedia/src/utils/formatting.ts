...
export function replaceLinks(text: string, language: string, links: string[] = []) {
  ...
  return text.replace(regex, (link: string) => {
    // Use encodeURI (not encodeURIComponent) because Wikipedia titles can contain
    // slashes (e.g. "AC/DC") that should be preserved in the URL path.
    const url = `https://${language}.wikipedia.org/wiki/${encodeURI(link)}`;
    return `[${link}](${url})`;
  });
}
...
const excludedMetatadataLabels = [
  "image",
  "cat",
  "subcat",
  "sortkey",
...
const excludedMetatadataValues = ["true", "false", "si", "no", "altura"];
