...
import JJWorkflow from "./workflow";
  ...
  return (
    <List>
        ...
        <List.Item
          ...
          actions={
            <ActionPanel>
              ...
              <Action
                title="Time Travel..."
                onAction={() => launchCommand(JJEdit, { "repo-path": currentRepo! })}
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={() => launchCommand(JJBookmarks, { "repo-path": currentRepo! })}
      ...
      <List.Section title="Quick Actions">
        {hasUncommittedChanges && !hasDescription && (
          <List.Item
            title="✨ Describe & Commit"
            subtitle="Add description to current changes"
            icon={Icon.Plus}
            actions={
              <ActionPanel>
                <Action
        ...
        {hasUncommittedChanges && hasDescription && (
          <List.Item
            title="🚀 Push Changes"
            subtitle="Push current changes to remote"
            ...
            actions={
              <ActionPanel>
                <Action
                  ...
                  onAction={async () => {
                    try {
                      const result = pushToGit(undefined, currentRepo);
                      await showToast({
                ...
                />
                <Action
                  title="Start Push Workflow..."
                  onAction={() => launchCommand(JJWorkflow, { "repo-path": currentRepo!, workflow: "push-only" })}
                  shortcut={{ modifiers: ["ctrl", "shift"], key: "p" }}
                />
                <Action
                  title="Create Bookmark First..."
                  onAction={() => launchCommand(JJBookmarks, { "repo-path": currentRepo! })}
        ...
        <List.Item
          ...
          subtitle="Pull latest changes and push local"
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                onAction={async () => {
                  try {
                    const result = pullFromGit(currentRepo);
                    await showToast({
              ...
              />
              <Action
                title="Start Sync Workflow..."
                onAction={() => launchCommand(JJWorkflow, { "repo-path": currentRepo!, workflow: "pull-push" })}
