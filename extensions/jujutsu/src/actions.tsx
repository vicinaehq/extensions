...
import { ActionPanel, Action, Icon, showToast, Clipboard, LaunchProps, LaunchType } from "@vicinae/api";
  ...
  static createCrossNavigation(repoPath: string, push: (component: any) => void, currentScreen?: string) {
    ...
    actions.push(
      <ActionPanel.Section title="Sync">
        <Action
          title="Pull & Push..."
          onAction={() => launchCommand(push, JJWorkflow, { "repo-path": repoPath, workflow: "pull-push" })}
          icon={Icon.ArrowRightCircle}
          shortcut={{ modifiers: ["ctrl", "shift"], key: "s" }}
        />
        <Action
          title="Pull Only..."
          onAction={() => launchCommand(push, JJWorkflow, { "repo-path": repoPath, workflow: "pull-only" })}
          icon={Icon.ArrowDownCircle}
          shortcut={{ modifiers: ["ctrl", "shift"], key: "p" }}
        />
        <Action
          title="Push Only..."
          onAction={() => launchCommand(push, JJWorkflow, { "repo-path": repoPath, workflow: "push-only" })}
