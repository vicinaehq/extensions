...
function formatResetsIn(isoTime?: string): { text: string | null; seconds: number | null } {
  if (!isoTime) return { text: null, seconds: null };

  const resetDate = new Date(isoTime);
  if (Number.isNaN(resetDate.getTime())) return { text: null, seconds: null };

  const diffMs = resetDate.getTime() - Date.now();
  if (diffMs <= 0) return { text: "now", seconds: 0 };

  const diffMinutes = Math.floor(diffMs / 60000);
  if (diffMinutes < 60) return { text: `${diffMinutes}m`, seconds: Math.floor(diffMs / 1000) };

  const hours = Math.floor(diffMinutes / 60);
  const minutes = diffMinutes % 60;
  if (hours < 24) return { text: minutes > 0 ? `${hours}h ${minutes}m` : `${hours}h`, seconds: Math.floor(diffMs / 1000) };

  const days = Math.floor(hours / 24);
  const remHours = hours % 24;
  return { text: remHours > 0 ? `${days}d ${remHours}h` : `${days}d`, seconds: Math.floor(diffMs / 1000) };
}
...
// Linux Secret Service (gnome-keyring) functions
...
export function readClaudeCredentials(): { credentials: ClaudeCredentials | null; error: ClaudeError | null } {
  ...
  for (const credentialsPath of resolveClaudeCredentialsPaths()) {
    ...
    try {
      ...
      // Fall through to platform-specific credential store
    }
  }

  // Strategy 2: Platform-specific credential store fallback
  let credentialValue: string | null = null;
  let credentialAccount: string | undefined = undefined;
  let credentialSource: CredentialSource = "file";

  if (process.platform === "darwin") {
    // macOS: Use Keychain
    credentialValue = readKeychainPassword(KEYCHAIN_SERVICE);
    if (credentialValue) {
      credentialAccount = readKeychainAccount(KEYCHAIN_SERVICE) ?? undefined;
      credentialSource = "keychain";
    }
  } else if (process.platform === "win32") {
    // Windows: Use Credential Manager
    credentialValue = readWindowsCredential(KEYCHAIN_SERVICE);
    if (credentialValue) {
      credentialAccount = "claude";
      credentialSource = "file"; // Treat as file-like for persistence
    }
  } else if (process.platform === "linux") {
    // Linux: Use Secret Service (gnome-keyring/kwallet)
    credentialValue = readLinuxSecret(KEYCHAIN_SERVICE);
    if (credentialValue) {
      credentialAccount = "claude";
      credentialSource = "file"; // Treat as file-like for persistence
    }
  ...
  if (credentialValue) {
    const parsed = tryParseCredentialJSON(credentialValue);
    if (parsed?.claudeAiOauth?.accessToken) {
      return extractCredentials(parsed, credentialSource, undefined, credentialAccount);
    }
  }
...
function persistRefreshedCredentials(credentials: ClaudeCredentials, refreshed: OAuthRefreshResponse) {
  ...
  if (credentials.source === "keychain") {
    ...
    // Windows: Try to persist to Credential Manager, fall back to file
    const credentialValue = JSON.stringify(next);
    writeWindowsCredential(KEYCHAIN_SERVICE, credentialValue);
    ...
    // Linux: Try to persist to Secret Service, fall back to file
    const credentialValue = JSON.stringify(next);
    writeLinuxSecret(KEYCHAIN_SERVICE, credentialValue);
    // Also persist to file as backup
    try {
      const credentialsPath = credentials.credentialsPath ?? resolveClaudeCredentialsPaths()[0];
      fs.writeFileSync(credentialsPath, `${JSON.stringify(next, null, 2)}\n`, "utf-8");
  ...
  try {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (window && typeof window.utilization === "number") {
        ...
        const resets = formatResetsIn(window.resets_at);
        modelWindows[modelName] = {
          percentageRemaining: clampPercent(100 - window.utilization),
          resetsIn: resets.text,
          resetsInSeconds: resets.seconds,
        };
      }
    }
    ...
    if (Array.isArray(data.limits)) {
      for (const limit of data.limits) {
        ...
        const resets = formatResetsIn(limit.resets_at);
        modelWindows[key] = {
    ...
    const usage: ClaudeUsage = {
      ...
      fiveHour: {
        ...
        resetsIn: formatResetsIn(fiveHour.resets_at).text,
        resetsInSeconds: formatResetsIn(fiveHour.resets_at).seconds,
      },
      sevenDay:
        sevenDay && typeof sevenDay.utilization === "number"
          ? {
              percentageRemaining: clampPercent(100 - sevenDay.utilization),
              resetsIn: formatResetsIn(sevenDay.resets_at).text,
              resetsInSeconds: formatResetsIn(sevenDay.resets_at).seconds,
            }
          : null,
      modelWindows,
      extraUsage,
