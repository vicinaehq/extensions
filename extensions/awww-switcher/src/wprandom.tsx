import { Cache, showToast, Toast, getPreferenceValues } from "@vicinae/api";
import {
  getImagesFromPath,
  Image,
  processImage,
  wallpaperSourceChanged,
} from "./utils/image";
...
export default async function RandomWallpaper() {
  ...
  try {
    ...
    function getRandomOrder(): string | undefined {
      return cache.get("randomOrder");
    }
    ...
    async function getRandonmWallpaperIndex(): Promise<number> {
      ...
      if (
        ...
        (await wallpaperSourceChanged(pathExpanded))
      ) {
        ...
        cache.set("randomOrder", randomOrderArray.join(" "));
      }
      ...
      if (spaceIndex != -1) {
        ...
        cache.set("randomOrder", randomOrder!.slice((spaceIndex ?? -1) + 1));
      } else {
        ...
        do {
          randomOrderArray = generateRandomOrderArray();
        } while (index == randomOrderArray[0]);
        cache.set("randomOrder", randomOrderArray.join(" "));
      }
      return index;
    }
    ...
    const wallpaperInfo = await processImage(
