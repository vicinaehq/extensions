...
import { detectVariant, getFavicon, variantInfo } from "./utils";
...
export default function Command() {
	...
	const openScheme = variant ? variantInfo(variant).openScheme : "brave";

	useEffect(() => {
		if (!variant) {
			setEngine(null);
			setEngineError("No Brave or Brave Origin installation detected");
			return;
		}
		void getSearchEngine(variant).then(setEngine).catch((e) => setEngineError(e instanceof Error ? e.message : String(e)));
	}, []);
	...
	const engineName = engine?.name ?? "Brave Search";
	...
	return (
		...
		>
			{engineError ? (
				<List.EmptyView title="Could not determine Brave's search engine" description={engineError} icon={Icon.Exclamationmark} />
			) : (
				<>
					{items.map((item) => (
						<List.Item
								...
								<ActionPanel>
									<Action
										...
										onAction={async () => {
											...
											await open(item.url, openScheme);
										}}
									/>
					...
					<List.EmptyView
						...
						description={`Search with ${engineName}, Brave's default search engine`}
