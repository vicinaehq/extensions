...
- Internal refactor: split `item-utils.ts` into focused modules (`toast`, `item-list`, `item-actions`, `item-form`); collapsed 22 defensive catch blocks in `bw-executor.ts` to two conversion seams; deleted single-use `json-utils.ts` in favour of inlined validators; extracted `useCardFields`, `runInitialLoad`, `validateSendForm`, `requireApiCreds`, and `parseDateFromHours`; replaced `getLoginActions` push ladder with a declarative spec.
- Enabled `noUncheckedIndexedAccess` and `noImplicitOverride` in `tsconfig.json`.
- CI: dropped `--hotspots` from the fallow health badge so the score reflects static code health rather than churn-weighted complexity.
