...
export default function GnomeSettingsList() {
  ...
  const openPanel = async (panelName: string) => {
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || result.stderr || "Failed to launch GNOME Control Center");
      }
  ...
  return (
    ...
    >
      <List.Section title={`Available GNOME Settings Panels (${filteredPanels.length})`}>
