...
export function EntryActions({ entry, prefs }: { entry: Entry; prefs: Preferences }) {
  return <ActionPanel>
    <Action title="Open in Zen" icon={Icon.Globe} onAction={async () => { runZen([entry.url], prefs); await showToast({ style: Toast.Style.Success, title: "Opening in Zen", message: entry.url }); }} />
