...
async function main(): Promise<void> {
	...
	try {
		...
		if (result.failedCount) throw new Error(result.outcomes[0]?.error ?? "Dispatch failed");
		...
		process.stdout.write("Verified transient systemd helper and Snooze storage lifecycle\n");
	} finally {
		if (reminderId) {
			await run(
				systemctl,
