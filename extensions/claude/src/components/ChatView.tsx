...
export function ChatView({ chat: initialChat }: { chat: Chat }) {
	...
	// Auto-focus the last message when messages are added
	useEffect(() => {
		if (chat.messages.length > 0) {
			const lastMessage = chat.messages[chat.messages.length - 1];
			setSelectedItemId(lastMessage.id);
		} else if (streamingMessage !== null) {
			setSelectedItemId("streaming-message");
		}
	}, [chat.messages.length, streamingMessage]);
