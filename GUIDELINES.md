...
- Always use `npm install` to install dependencies and generate the `package-lock.json`, and commit the resulting lockfile. This is required: our CI and the Nix build install strictly from `package-lock.json`, so a lock generated by another package manager (bun, pnpm) or left out of sync with `package.json` will fail the build. It is fine to use `bun` to run scripts during development (e.g. `bun run dev`), just not to install dependencies.
